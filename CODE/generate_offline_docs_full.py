...
def filter_relevant_paragraphs(doc, name, keywords=None):
    ...
    if doc != 'empty':  # this means that soap returned empty text
        paragraphs = [p.strip() for p in doc.split("\n") if len(p.strip()) > 20] # BID: Commented this out
        #paragraphs = [p.strip() for p in doc.split("\n") if p.strip()] # BID: Changed this to remove the length check
        print(paragraphs) 
        if paragraphs != []:
...
def generate_offline_file(data, file_name_prefix):
    ...
    for person in data:
        ...
        if links != []:
            raw_documents = retrieve_documents(links, name)
        else:
            print(f"⚠️ No links found for '{name}', skipping.")
            raw_documents = []  # or continue
            #raise Exception("Quota exceeded for quota metric 'Queries' and limit 'Queries per day'")

        rows.append([name, raw_documents])

    df = pd.DataFrame(rows, columns=['full_name', 'raw_documents'])
    file_path = './offline_data/' + file_name_prefix + '_offline_raw_documents_real_strict.csv'
    ...
    print(f"✅ Raw documents saved to {file_path}")

    
if __name__ == "__main__":
    real_data_name = "Namensliste CEOs und BoDs Nebenwerte Schweiz neu.csv"
    ...
    #generate_offline_file(real_data[0:1], safe_name)
